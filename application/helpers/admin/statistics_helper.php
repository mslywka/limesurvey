...
class statistics_helper {
    ...
    */
	...
	$qgid = "";
        ...
        return array("alist"=>$alist, "qtitle"=>$qtitle, "qquestion"=>$qquestion, "qtype"=>$qtype, "statisticsoutput"=>$statisticsoutput, "parentqid"=>$qqid, "gid"=>$qgid);
    }
    ...
    */
    protected function displayResults($outputs, $results, $rt, $outputType, $surveyid, $sql, $usegraph, $browse, $oLanguage) {
		...
		// Using the sid and gid pull the question group from the database
                $nquery = "SELECT group_name FROM {{groups}} WHERE sid='{$msid}' AND gid='{$mgid}'";
                $nresult = Yii::app()->db->createCommand($nquery)->query();
		...
		//rru-specific end
                ...
                //$pdfTitle = $this->pdf->delete_html(sprintf($statlang->gT("Question Group3: %s"),$group_name));
                $pdfTitle = $this->pdf->delete_html(sprintf($statlang->gT("%s"),html_entity_decode($group_name,ENT_QUOTES,'UTF-8')));
                ...
                if($cachefilename) // Add the image only if constructed
                {
                    ...
                    switch($outputType)
                    {
                        ...
                        case 'pdf':
                            ...
                            //rru-specific start
                            //Put the graph on the same page as the statistics and remove hyper-link to survey
                            $this->pdf->Image($tempdir."/".$cachefilename, 0, 150, 180, 0, '', '', 'B', true, 150,'C',false,false,0,true);
                            //rru-specific end
    ...
    public function generate_statistics($surveyid, $allfields, $q2show='all', $usegraph=0, $outputType='pdf', $pdfOutput='I',$statlangcode=null, $browse = true)
    {
        ...
        */
        if($outputType=='pdf')
        {
            ...
            //rru-specific start
            $headerlogo = 'rru_logo.jpg';
            $this->pdf->SetHeaderData($headerlogo, 25, $statlang->gT("Survey Results for:",'unescaped') , flattenText($surveyInfo['surveyls_title'],false,true,'UTF-8'));
            //rru-specific end
        ...
        */
        ...
        switch($outputType)
        {
            ...
            case 'pdf':
                ...
                //rru-specific start
                $this->pdf->titleintopdf($statlang->gT("Survey",'unescaped')." ".$surveyid." ".$surveyInfo['surveyls_title']);
                //$this->pdf->titleintopdf($statlang->gT("Survey",'unescaped')." ".$surveyid);
		...
		{
			//rru-specific start
		        /*$fieldmap=createFieldMap($surveyid, "full", true, false, "en");
		        foreach ($fieldmap as $field)
		        {
			   if(isset($field['qid']) && $field['qid']!='')
                           {
...
/*
			...
			// Using the sid and gid pull the question group from the database
                        ...
                        //loop though question data
                        //foreach ($nresult->read() as $nrow)
                        foreach ($nresult->readAll() as $nrow)
			{
				//$outputs['group_name']=$nrow;
				//
                            $nrow=array_values($nrow);
